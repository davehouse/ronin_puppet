...
class roles_profiles::profiles::gecko_t_osx_1100_generic_worker (
    String $worker_type = 'gecko-t-osx-1100-bug1668885',
) {
    ...
    case $::operatingsystem {
        'Darwin': {
            ...
            class { 'puppet::atboot':
                ...
                puppet_branch       => 'bug1668885_macos-bigsur-testers',
            ...
            $taskcluster_client_id    = lookup('generic_worker.datacenter_gecko_t.taskcluster_client_id')
            $taskcluster_access_token = lookup('generic_worker.datacenter_gecko_t.taskcluster_access_token')
            $livelog_secret           = lookup('generic_worker.datacenter_gecko_t.livelog_secret')
            $quarantine_client_id     = lookup('generic_worker.datacenter_gecko_t.quarantine_client_id')
            $quarantine_access_token  = lookup('generic_worker.datacenter_gecko_t.quarantine_access_token')
            $bugzilla_api_key         = lookup('generic_worker.datacenter_gecko_t.bugzilla_api_key')
